...
def contact():
  if request.method == "POST":
    ...
    db.session.commit()
    #mail.send_message('New mail from ' + name,  sender=email, recipients=['email123@gmail.com'], body=message+"\n"+phone)
...
def edit(sno):
    ...
    if request.method=='POST':
      ...
      date = datetime.now()
      if sno=="0":
         post =  Posts(sno= "0",title=box_title,tagline=tline,slug=slug,content=content,img_file=img_file,date=date)
         db.session.add(post)
         db.session.commit()
      else:
